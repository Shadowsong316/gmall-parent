...
public class GmallRabbitConfig {
    ...
    public Queue delayOrderQueue(){
        ...
        Queue queue = new Queue("userOrderQueue", true, false, false,arguments);
        return queue;
    }
    /************************绑定关系************************/
    @Bean
    public Binding stockOrderQueueBinding(){
        return new Binding("stockOrderQueue",Binding.DestinationType.QUEUE,
                "orderFanoutExchange","",null);
    }
    @Bean
    public Binding userOrderQueueBinding(){
        return new Binding("userOrderQueue",Binding.DestinationType.QUEUE,
                "orderFanoutExchange","",null);
    }
    @Bean
    public Binding delayOrderQueueBinding(){
        return new Binding("delayOrderQueue",Binding.DestinationType.QUEUE,
                "orderFanoutExchange","",null);
    }
    @Bean
    public Binding deadOrderQueueBinding(){
        return new Binding("deadOrderQueue",Binding.DestinationType.QUEUE,
                "orderDeadExchange","dead.order",null);
    }
    /************************绑定关系************************/
    @Bean
